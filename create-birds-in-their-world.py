#!/usr/bin/env python3
"""Create Birds in Their World page."""
...
def parse_filename(filename):
    ...
    if len(parts) >= 3:
        ...
        # Species is typically 1-2 words
        # Location can be multiple words
        # Title/caption is the rest

        # For american-oystercatcher-barnegat-state-park-over-the-dune:
        species = 'American Oystercatcher'
        location = 'Barnegat State Park'
        title = 'Over the Dune'

        return species, location, title
...
# Create the Birds in Their World page
print("\nCreating birds-in-their-world.html...")
...
<head>
    ...
    <title>Birds in Their World - Bird Photography</title>
...
<body>
    ...
    <nav>
        <ul>
            ...
            <li><a href="birds-in-their-world.html">Birds in Their World</a></li>
...
{gallery_section}
    ...
    <script>
        ...
        images = Array.from(worldImages).map((img, index) => {{
            const item = img.closest('.world-item');
            return {{
                src: img.src,
                alt: img.alt,
                species: item.querySelector('.world-species').textContent,
                location: item.querySelector('.world-location').textContent,
                title: item.querySelector('.world-title').textContent
            }};
        }});

        // Add click handlers
        worldImages.forEach((img, index) => {{
        ...
        function showImage() {{
            ...
            lightboxCaption.querySelector('.species').textContent = image.species;
            lightboxCaption.querySelector('.location').textContent = image.location;
            lightboxCaption.querySelector('.title').textContent = image.title;
        }}
...
print("✓ Created birds-in-their-world.html")
print(f"\n✓ Birds in Their World page complete with {len(images)} images!")
